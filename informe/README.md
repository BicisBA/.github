...
- Las predicciones requieren que el cliente provea en cuantos minutos va a llegar el usuario a la estación. Para esto, la aplicación web toma un valor por defecto de que una persona promedio camina 5 kilometros por hora (sacado de distintas fuentes). Esto no es óptimo, porque cuanto tarda una persona en llegar a otro punto depende de miles de factores más (si está en subida, si está cansado, cuán rápido camina, etc). Idealmente, deberíamos usar un servicio especializado en consultas de ruteo, como lo es [OSRM](http://project-osrm.org/). Incorporar esta integración requiere que uno auto disponga de su instancia de OSRM, lo cual involucra un trabajo muy superior al scope planteado inicialmente.
